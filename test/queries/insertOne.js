...
describe('QUERY insertOne', () => {
    ...
    it('should insert given entities when executed', function* () {
        const insertOne = insertOneQuerier('author', ['name', 'firstname']);
        const result = (yield db.query(insertOne({ name: 'doe', firstname: 'john' })))[0];

        assert.equal(result.name, 'doe');
        assert.equal(result.firstname, 'john');
        const insertedEntity = (yield db.query({
