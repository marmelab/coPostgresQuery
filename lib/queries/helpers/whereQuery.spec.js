...
describe('whereQuery', () => {
    it('should return whereQuery', () => {
        ...
        }, ['column1', 'column2', 'column3', 'column4', 'column5', 'table.column6', 'column7', 'column9']), [
    ...
    describe('getColPlaceHolder', () => {
        ...
        it('should return IS NULL if value is null', () => {
            assert.equal(whereQueryGet.getColPlaceHolder('colName', null), 'IS NULL');
        });

        it('should return IS NOT NULL if value is null and not is true', () => {
            assert.equal(whereQueryGet.getColPlaceHolder('colName', null, true), 'IS NOT NULL');
        });
        ...
        it('should return NOT IN query if value is an array and not is true', () => {
            assert.equal(
                whereQueryGet.getColPlaceHolder('colName', ['some value', 'other value'], true),
                'NOT IN ($colName1, $colName2)',
            );
        });

        it('should return != $colName if not is true', () => {
            assert.equal(
                whereQueryGet.getColPlaceHolder('colName', 'some value', true),
                '!= $colName',
            );
        });
    ...
    describe('getColType', () => {
        ...
        it('should return like if column is suffixed by like and is in searchableCols', () => {
            assert.equal(whereQueryGet.getColType('like_column', ['column']), 'like');
        });

        it('should return discarded if column is suffixed by like but is not in searchableCols', () => {
            assert.equal(whereQueryGet.getColType('like_column', ['other_column']), 'discarded');
        });

        it('should return not like if column is suffixed by not_like and is in searchableCols', () => {
            assert.equal(whereQueryGet.getColType('not_like_column', ['column']), 'not like');
        });

        it('should return discarded if column is suffixed by not_like but is not in searchableCols', () => {
            assert.equal(whereQueryGet.getColType('not_like_column', ['other_column']), 'discarded');
        });
    });
    ...
    describe('getNotLike', () => {
        it('should return query and parameter to test column not like value if given column starting with not_like_', () => {
            const whereParts = whereQueryGet.getNotLike({ not_like_column: 'pattern' }, ['column']);

            assert.deepEqual(whereParts, ['column::text NOT ILIKE $not_like_column']);
        });

        it('should augment passed result if any', () => {
            const whereParts = whereQueryGet.getNotLike(
                { not_like_column: 'pattern' },
                ['column'],
                ['column1 = $column1'],
            );

            assert.deepEqual(whereParts, ['column1 = $column1', 'column::text NOT ILIKE $not_like_column']);
        });
    });

    describe('getNot', () => {
        it('should return query and parameter to test column not equal to value if given column starting with not_', () => {
            const whereParts = whereQueryGet.getNot({ not_column: 'not me' }, ['column']);

            assert.deepEqual(whereParts, ['column != $not_column']);
        });

        it('should augment passed result if any', () => {
            const whereParts = whereQueryGet.getNot(
                { not_column: 'not me' },
                ['column'],
                ['column1 = $column1'],
            );

            assert.deepEqual(whereParts, ['column1 = $column1', 'column != $not_column']);
        });
    });
