import configurable from 'configurable.js';
import whereQuery from './whereQuery';
import sanitizeParameter from './sanitizeParameter';
import flattenParameters from './flattenParameters';

function selectPage(limit, offset, filters = {}, sort, sortDir) {
    const {
        table,
        returnFields,
        idFields,
        searchableFields,
        specificSorts,
        groupByFields,
        withQuery,
    } = this.config;

    const where = whereQuery(filters, searchableFields);
    let sql = `SELECT ${returnFields.join(', ')} FROM ${table} ${where}`;

    if (groupByFields.length > 0) {
        sql = `${sql} GROUP BY ${groupByFields.join(', ')}`;
    }
    // withQuery add a temporary result table that allow to filters on computed and joined field
    if (withQuery) {
        sql = (
`WITH result AS (
${sql.trim()}
) SELECT * FROM result`
);
    }

    const parameters = flattenParameters(sanitizeParameter(searchableFields, filters));

    // always sort by ids to avoid randomness in case of identical sortField value
    const sortQuery = [idFields.map(idField => `${idField} ASC`).join(', ')];
    if (sort) {
        const normalizedSort = sort.toLowerCase();
        if (specificSorts && specificSorts.hasOwnProperty(normalizedSort)) {
            const specificSort = specificSorts[normalizedSort]
            .reduce(
                (result, condition, index) =>
                    `${result} WHEN '${condition}' THEN ${index + 1}`, `CASE ${normalizedSort}`
            );
            sortQuery.unshift(
                `${specificSort} END ${(sortDir.toLowerCase() === 'asc' ? ' ASC' : ' DESC')}`
            );
        } else if (returnFields.indexOf(normalizedSort) !== -1) {
            sortQuery.unshift(
                `${normalizedSort} ${(sortDir.toLowerCase() === 'asc' ? 'ASC' : 'DESC')}`
            );
        }
    }
    sql = `${sql} ORDER BY ${sortQuery.join(', ')}`;

    if (limit) {
        sql = `${sql} LIMIT $limit OFFSET $offset`;
        parameters.limit = limit || 30;
        parameters.offset = offset || 0;
    }

    return { sql, parameters };
}

export default ({ table, idFields, returnFields, searchableFields, specificSorts, groupByFields, withQuery } = {}) =>
    configurable(selectPage, {
        table,
        idFields: [].concat(idFields),
        returnFields,
        searchableFields: searchableFields || returnFields,
        specificSorts: specificSorts || {},
        groupByFields: groupByFields || [],
        withQuery: withQuery || table.indexOf('JOIN') !== -1,
    });
